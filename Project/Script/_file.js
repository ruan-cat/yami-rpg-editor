...
Directory.update = function () {
	...
	const { assets } = this
	if (assets !== null && this.updating === null) {
		Meta.versionId++
		this.reading = true
		this.updating = assets
			.update()
			.then(
...
Directory.getFile = function (path) {
	...
	if (folder.path === dirname) {
		for (const item of folder.children) {
			if (item.path === path && item instanceof FileItem) {
				return item
			}
		}
	}
	...
	const check = async (dirPath, dir) => {
		...
		for (const file of dir) {
			...
			if (file.children?.length) {
				promises.push(check(path, file.children))
			}
		}
		if (promises.length !== 0) {
	...
	return function (files) {
		...
		while (--i >= 0) {
			...
			for (const folder of folders) {
				if (path.indexOf(folder) === 0 && path[folder.length] === '/') {
					files.splice(i, 1)
					continue
				}
			}
...
Directory.moveFiles = (function IIFE() {
	...
	return function (dirPath, dir) {
		return move(dirPath, Directory.filterFiles(dir), {})
	}
})()
